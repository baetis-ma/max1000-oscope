...
//these must be known 
int samples=500, chcnt=2, trigger=1, trigoff;
...
void menu(int fd)
{
   ...
   if(kbhit()){
      ...
      if (temp == 'h'){
         ...
         fprintf(stderr, "t - toggle trigger\n");
      ...
      if (temp == 't'){
         if (trigger == 1) trigger = 0; else trigger = 1;
         r51 = trigger*0x8000 + chcnt;
         sprintf(tout, "w 51 %04x", r51);    
         tout[strlen(tout)] = '\x0d';
         tout[strlen(tout)] = '\0';
         write(fd, tout, strlen(tout));
         fprintf(stderr, "%s   ", tout);
         if(trigger==0)fprintf(stderr, "trigger off\n");
                 else  fprintf(stderr, "trigger on\n");
         tcdrain(fd);
      }
      if (temp == 's'){
...
{ 
    ...
    printf("array xa[2048]\n");
    printf("array y1a[2048]\n");
    printf("array y2a[2048]\n");
    printf("array y3a[2048]\n");
    printf("array y4a[2048]\n");
    ...
    while(1) {
     ...
     if (strobe == 4) {
        while(strobe == 4 && read(fd,byte,1)!= 0)  {
           ...
           menu(fd);
           cnt = cnt + 1;
           if (cnt == 1) {trigger = byte[0]/128; chcnt = byte[0]%8; }
           ...
           if(cnt==header){printf("set yrange[-0.5:%d]\n",4+off*4*(chcnt-1));
                       ...
                       if (trigger == 1) printf("set label 1 \"*\" at %f,-0.25 center tc rgb \'red\'\n",foff*ftime);  
